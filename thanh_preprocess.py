...
def mezhang_news_preprocess(args):
    ...
    data_path = os.path.join(args.root_data_dir, args.dataset)
    ...
    for l in tqdm(open(os.path.join(args.root_data_dir, args.dataset, "train/news.tsv"), "r", encoding='utf-8')):
        nid, vert, subvert, title, abst, url, ten, aen = l.strip("\n").split("\t")
        if nid in nid2index:
            continue
    ...
    for l in tqdm(open(os.path.join(args.root_data_dir, args.dataset, "valid/news.tsv"), "r", encoding='utf-8')):
        nid, vert, subvert, title, abst, url, ten, aen = l.strip("\n").split("\t")
        if nid in nid2index:
            continue
        title = word_tokenize(title)[:max_title_len]
    ...
    if os.path.exists(os.path.join(data_path , "test") ):
        ...
        for l in tqdm(open(os.path.join(args.root_data_dir, args.dataset, "test/news.tsv"), "r", encoding='utf-8')):
            nid, vert, subvert, title, abst, url, ten, aen = l.strip("\n").split("\t")
...
def news_preprocess(args):
    ...
    out_path = os.path.join(args.root_dir, args.root_data_dir, args.dataset, 'utils')
    ...
    # else:
    read_news(args, os.path.join(out_path, "train/news.tsv"))
    read_news(args, os.path.join(out_path, "valid/news.tsv"))
    read_news(args, os.path.join(out_path, "test/news.tsv"))
    ...
    glove_path = os.path.join(args.root_dir, args.root_data_dir, "glove/glove.6B.300d.txt")
    embedding_matrix, exist_word = load_matrix(glove_path, vocab_dict)
...
def split_then_select_behavior_preprocess(args):
    ...
    print('Preprocessing for Simulator ...') 
    read_imprs(args, os.path.join(args.root_dir, args.root_data_dir, args.dataset, "utils/train/behaviors.tsv"), 0, save=True)

    read_imprs(args, os.path.join(args.root_dir, args.root_data_dir, args.dataset, "utils/valid/behaviors.tsv"), 1, save=True)

    train_user_set, _, tr_rep_sorted_samples, _ = \
        read_imprs(args, os.path.join(args.root_dir, args.root_data_dir, args.dataset, "utils/train/behaviors.tsv"), 1) 
...
def generate_cb_news(args):
    ...
    train_news_path = os.path.join(args.root_dir, args.root_data_dir, args.dataset, "utils/train/news.tsv") 
    valid_news_path = os.path.join(args.root_dir, args.root_data_dir, args.dataset, "utils/valid/news.tsv")
    ...
    save_path = os.path.join(args.root_dir, args.root_data_dir, args.dataset, "utils/cb_news.pkl") 
...
def compute_empirical_ips(args): 
    ...
    nid2index, word2vec, nindex2vec = load_word2vec(args)
    data_path = os.path.join(args.root_dir, args.root_data_dir, args.dataset, 'utils')
...
if __name__ == "__main__":
    ...
    read_imprs_for_val_set_for_sim(args, os.path.join(args.root_dir, args.root_data_dir, args.dataset, "utils/valid/behaviors.tsv"))
